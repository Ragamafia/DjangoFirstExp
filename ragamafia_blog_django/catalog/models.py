...
class Genre(models.Model):   # Модель жанра книги
    ...
    name = models.CharField(max_length=200, help_text="Введите жанр книги (например, научная фантастика, документальная литература)")
...
class BookInstance(models.Model):   # Модель, представляющая конкретный экземпляр книги (т.е. ее можно взять в библиотеке)
